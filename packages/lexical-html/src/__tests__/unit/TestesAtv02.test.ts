  ...
  test(`[Lexical -> HTML]: Default selection (undefined) should serialize entire editor state`, () => {
    ...
    editor.update(
      () => {
        ...
        const text2 = $createTextNode('World');
        p2.append(text2);
        const text3 = $createTextNode(' O Windows PowerShell');
        const text4 = $createTextNode(' mais uma vez');

        p2.append(text3).append(text4);
        root.append(p1).append(p2);
    ...
    expect(html).toBe(
      '<p><span>Hello</span></p><p><span>World O Windows PowerShell mais uma vez</span></p>',
    );
  });
});
